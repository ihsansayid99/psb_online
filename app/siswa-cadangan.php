...
if (isset($_GET['id'])) { // memperoleh
    ...
    if (!empty($id_pendaftar)) {
        // Query
        $sql = "SELECT * FROM pendaftaran WHERE id = '{$id_pendaftar}' AND status_pendaftaran = 'cadangan';";
...
?>
        ...
        <head>
            ...
            <title>SURAT KETERANGAN LULUS-CADANGAN</title>
            <style>
                * {
                    margin: 0;
...
<?php
        ...
        $dompdf->render();
        ob_end_clean();
        $dompdf->stream("Keterangan-Cadangan-" . $id_pendaftar, array("Attachment" => false));
        exit();
    }
}
?>