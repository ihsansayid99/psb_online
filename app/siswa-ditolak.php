...
if (isset($_GET['id'])) { // memperoleh
    ...
    if (!empty($id_pendaftar)) {
        // Query
        $sql = "SELECT * FROM pendaftaran WHERE id = '{$id_pendaftar}' AND status_pendaftaran = 'tidak-diterima';";
...
?>
        ...
        <head>
            ...
            <title>SURAT KETERANGAN TIDAK LULUS</title>
            <style>
                * {
                    margin: 0;
        ...
        <body>
            ...
            ?>
                <section id="member-card">
                    <h3>KOP SEKOLAH</h3>
                    <h3>SURAT KETERANGAN TIDAK LULUS SMK NEGERI 1 JAKARTA</h3>
...
<?php
        ...
        $dompdf->render();
        ob_end_clean();
        $dompdf->stream("Keterangan-Ditolak-" . $id_pendaftar, array("Attachment" => false));
        exit();
    }
}
?>